...
public sealed class DoctorAvailabilityChangedSearchIndexHandlerTests
{
    [Fact]
    public async Task Handle_WhenDoctorIsVerified_UpsertsDoctorIndex()
    {
        ...
        var handler = new DoctorAvailabilityChangedSearchIndexHandler(
            doctorRepository.Object,
            searchService,
            Mock.Of<Microsoft.Extensions.Logging.ILogger<DoctorAvailabilityChangedSearchIndexHandler>>());

        await handler.Handle(
            new DoctorAvailabilityChangedNotification(
                doctorId,
                DateTime.UtcNow),
